...
## dir ====
dir <- "/Volumes/EX_SSD/220517_cDNA_rrna/rrna/analysis/"

## libraries ====
packages <- c("ggeconodist", "tidyverse", "here", "ggthemes", "gganimate","writexl","seqinr","tximport",
              "colorblindr","rcartocolor","ChIPpeakAnno","tictoc","patchwork","psych","bioanalyzeR",
              "data.table", "ggExtra", "Rsamtools", "GenomicAlignments", "UpSetR","vroom", "ggsci",
...
## sample_names ====
bc_to_sample <- data.table(seq_run = "RNA_temp",
                           barcode = paste0(rep("barcode",12),sprintf("%02d", seq(1, 12))),
                           sample  = c("RNA_80_1",
                                       "RNA_80_2",
                                       "RNA_95_1",
                                       "RNA_95_2",
...
## read in gff file ====
old_to_new <- function(input_file){

read.gff(input_file) %>%
    dplyr::filter(type %in% c("gene")) %>%
    as_tibble() %>%
    dplyr::mutate(gene = str_split_fixed(str_split_fixed(attributes, "locus_tag=",2)[,2], ";",2)[,1],
                  old_gene = str_split_fixed(attributes, "old_locus_tag=",2)[,2]) %>%
    dplyr::select(gene, old_gene)
}
...
### sequence logos ####
acgt_color_scale = make_col_scheme(chars=c('A', 'U', 'C', 'G'), 
